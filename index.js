...
function processLineDirection(res2,id,dir,hs){
...
function processLine(id){
	...
	processLineDirection(res,id,0,res.direction_name_tur);

	res = request('GET', apiURL+'/rp/api/lines/'+id+'/direction/1');
	res = JSON.parse(res.getBody('utf8'));

	addShapes(res.segment_path,id+"1");
	processLineDirection(res,id,1,res.direction_name_retur);
}
